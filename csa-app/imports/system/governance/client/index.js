...
Template.governanceAdmin.helpers({
  ...
  members(){const users=new Map(Meteor.users.find({}).fetch().map((u)=>[u._id,userDisplay(u)]));return LodgeMemberships.find({},{sort:{matriculationNo:1}}).fetch().map((m)=>({...m,...(users.get(m.userId)||{displayName:m.userId,email:''}),_id:m._id}))},
