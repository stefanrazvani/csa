...
Meteor.publish('craft.confirmari.mine', async function mine(selectedId = '') {
  if (typeof selectedId !== 'string') return this.ready();
  ...
  return publishWithReactiveDossierAccess(this, {
    ...
    buildStreams: ({ eId, userId }) => [{ collection: PrezentaConfirmari, cursor: PrezentaConfirmari.find({ eId, userId, sys_status: 1, ...(selectedId ? {_id:selectedId} : {}) }, { fields: RESPONSE_FIELDS, sort: { dataTinuta: -1 } }) }],
  });
});

Meteor.publish('craft.attendance.workspace', async function workspace(eventId = '') {
  if (typeof eventId !== 'string') return this.ready();
  ...
  return publishWithReactiveDossierAccess(this, {
    ...
    buildStreams: ({ eId }) => [
      ...
      { collection: PrezentaConfirmari, cursor: PrezentaConfirmari.find({ eId, convocatorId: eventId, sys_status: 1 }, { fields: { ...RESPONSE_FIELDS, userSnapshot: 1, 'delivery.state': 1, 'delivery.sentAt': 1, 'delivery.error': 1 } }) },
    ],
  });
});
...
publishAuthorized('craft.convocatoare', async function publishConvocatoare() {
  const { eId, userId } = await requireRole(this, 'convocatoare', 'read');
  const grade = await getReadableCraftGrade(userId, eId).catch(() => 0);
  return Convocatoare.find({ eId, sys_status: 1, ...gradeSelector(grade, 'minGrade', true) }, { fields: CONVOCATOR_FIELDS, sort: { dataTinuta: -1, nr: -1 } });
});

publishAuthorized('craft.convocator', async function publishConvocator(id) {
  if (typeof id !== 'string') return this.ready();
  const { eId, userId } = await requireRole(this, 'convocatoare', 'read');
  const grade = await getReadableCraftGrade(userId, eId).catch(() => 0);
  return Convocatoare.find({ _id: id, eId, sys_status: 1, ...gradeSelector(grade, 'minGrade', true) }, { fields: CONVOCATOR_FIELDS });
});

publishAuthorized('craft.documenteText', async function publishDocumenteText(documentId) {
  ...
  const exists = await Convocatoare.findOneAsync({ _id: documentId, eId, sys_status: 1, ...gradeSelector(grade, 'minGrade', true) }, { fields: { _id: 1 } });
  if (!exists) return this.ready();
  return DocumenteText.find(
    { eId, documentId, sys_status: 1, ...gradeSelector(grade, 'level') },
    { sort: { level: 1, order: 1 } },
  );
});

publishAuthorized('craft.memberships', async function publishMemberships() {
...
publishAuthorized('craft.gradeAdmin', async function publishGradeAdmin() {
  const { eId } = await requireRole(this, 'convocatoare', 'admin');
  return [
...
publishAuthorized('craft.prezente', async function publishPrezente() {
  const { eId } = await requireRole(this, 'prezenta', 'read');
  return Prezenta.find({ eId, sys_status: 1 }, { sort: { dataTinuta: -1 } });
});

publishAuthorized('craft.confirmari', async function publishConfirmari(convocatorId) {
...
publishAuthorized('craft.documents', async function publishDocuments(objectId) {
  if (typeof objectId !== 'string') return this.ready();
  const { eId, userId } = await requireRole(this, 'documents', 'read');
  const grade = await getReadableCraftGrade(userId, eId);
  return Documente.find({ eId, objectId, sys_status: 1, ...gradeSelector(grade, 'minGrade', true) }, { fields: { privatePath: 0 } });
});
...
publishAuthorized('craft.libraryChoices', async function libraryChoices() {
